...
public class AllCaseTestEv extends UiAutomatorTestCase {
	...
	public static void main(String[] args){
		...
		String android_id = "1";
		 ...
		 String jar_name = "AllCaseTestEv";
		 //生成jar的名字
		 String test_class = "com.checheyun.vcm.VcmUiTest.AllCaseTestEv";
...
//shit shit,为什么进不了这个if
	...
	//学习mongodb了解业务后继续写点击任务流转人，校验车辆页面车辆信息，删除相关数据等操作（删除操作最好放在接车前）
				System.out.println(passCase+"xxxx");
				passCase=passCase+1;	
	...
	public void testJieChe2() throws UiObjectNotFoundException{
		...
		vcmlogginachieve();		
		UiObject AddCarBtn=new UiObject(new UiSelector().description("task_header_wrap").childSelector(new UiSelector().instance(3)));
		AddCarBtn.click();
		device.waitForIdle(1000);
		UiObject keyBoardBtn=new UiObject(new UiSelector().text("手动输入"));
		keyBoardBtn.clickAndWaitForNewWindow();
...
//点击关闭，后续补充
	...
	public void testJieChe6() throws UiObjectNotFoundException{
		...
		UiObject Btn1=new UiObject(new UiSelector().text("1"));
		Btn1.click();
		UiObject Btn2=new UiObject(new UiSelector().text("5"));
		Btn2.click();
		UiObject Btn4=new UiObject(new UiSelector().text("2"));
		Btn4.click();
		UiObject Btn5=new UiObject(new UiSelector().text("6"));
		Btn5.click();
		Btn1.click();
